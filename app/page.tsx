...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Real-time: actualiza agente cuando cambia status/tasks_done
  ...
  async function fetchAll() {
    const [ar, pr, tr, lr] = await Promise.all([
      // Prompts dinámicos: se leen desde Supabase (#7)
  ...
  return (
    <div className="dashboard-layout">
      ...
      <div className="main-content">
        ...
        {/* Stats — incluye métricas reales (#8) */}
        <div className="stats-grid">
          <StatCard label="Agentes activos" value={activeCount + busyCount} sub={`De ${agents.length} totales · ${busyCount} procesando`} accent={T.blue} />
          <StatCard label="Proyectos Jira" value={projects.length} sub={`${inProgTasks} en progreso · ${openTasks} por hacer`} accent={T.teal} />
          <StatCard label="Tareas completadas" value={totalTasksDone} sub={topAgent ? `Top: ${topAgent.icon} ${topAgent.name} (${topAgent.tasks_done})` : 'Sin tareas aún'} accent={T.blue} />
          <StatCard label="Tickets totales" value={tasks.length} sub={`${doneTasks} completados · ${openTasks} abiertos`} accent={T.teal} />
        </div>
